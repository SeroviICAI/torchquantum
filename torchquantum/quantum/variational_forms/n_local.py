...
class NLocalLayer(qu.Ansatz):
    ...
    def build_kernel(self) -> cudaq.Kernel:
        ...
        for idx in range(len(self.rotation_blocks)):
            ...
            rb_kernel: cudaq.Kernel = rb.build_kernel()
            kernel.apply_call(rb_kernel, qvec, *thetas[ptr : ptr + count])
            ptr += count
        ...
        for idx, gate in enumerate(self.entanglement_blocks):
            ...
            eb_kernel: cudaq.Kernel = eb.build_kernel()
            kernel.apply_call(eb_kernel, qvec)
        ...
        if self.apply_final_rotation:
            for idx in range(len(self.rotation_blocks)):
                ...
                frb_kernel = frb.build_kernel()
                kernel.apply_call(frb_kernel, qvec, *thetas[ptr : ptr + count])
                ptr += count
        return kernel
...
class NLocal(qvf.VariationalForm):
    ...
    def build_kernel(self) -> cudaq.Kernel:
        ...
        for layer in range(self.num_layers):
            ...
            layer_ansatz: NLocalLayer = NLocalLayer(
                self.num_qubits,
            ...
            kernel.apply_call(layer_kernel, qvec, *thetas[ptr : ptr + count])
            ptr += count
        return kernel
