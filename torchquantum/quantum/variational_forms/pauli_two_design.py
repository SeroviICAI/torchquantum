...
class PauliTwoDesignLayer(qu.Ansatz):
    ...
    def build_kernel(self) -> cudaq.Kernel:
        ...
        rb_kernel = rb.build_kernel()
        kernel.apply_call(rb_kernel, qvec, *thetas[: ptr + count])
        ptr += count
        ...
        if self.apply_final_rotation:
            ...
            kernel.apply_call(frb_kernel, qvec, *thetas[ptr:])
            ptr += count
        return kernel
...
class PauliTwoDesign(qvf.VariationalForm):
    ...
    def build_kernel(self) -> cudaq.Kernel:
        ...
        kernel, qvec, *thetas = cudaq.make_kernel(
            cudaq.qvector, *([float] * total_params)
        )
        ...
        for layer in range(self.num_layers):
            ...
            layer_kernel = ptd_layer.build_kernel()
