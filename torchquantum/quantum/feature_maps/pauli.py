...
class PauliBlock(qu.Ansatz):
    ...
    def build_kernel(self) -> cudaq.Kernel:
        ...
        kernel, qvec, *params = cudaq.make_kernel(
            cudaq.qvector, *([float] * self.get_parameter_count())
        )

        def self_product(vals: List[float]) -> float:
        ...
        if non_identity:
            kernel.rz(
                self.alpha * self_product(list(params)),
    ...
    def get_parameter_count(self) -> int:
        ...
        """
        return sum(1 for ch in self.pauli_string if ch != "I")
...
class PauliFeatureMapLayer(qu.Ansatz):
    ...
    def build_kernel(self) -> cudaq.Kernel:
        ...
        """
        total_params: int = self.get_parameter_count()
        kernel, qvec, *x = cudaq.make_kernel(cudaq.qvector, *([float] * total_params))
...
class PauliFeatureMap(qfm.FeatureMap):
    ...
    def build_kernel(self) -> cudaq.Kernel:
        ...
        """
        layer_total_params: int = PauliFeatureMapLayer(
            self.num_qubits, self.paulis, self.entanglement, self.alpha
        ).get_parameter_count()
        total_params: int = self.num_layers * layer_total_params
        kernel, qvec, *x = cudaq.make_kernel(cudaq.qvector, *([float] * total_params))
        ptr: int = 0
        for layer in range(self.num_layers):
            layer_ansatz: PauliFeatureMapLayer = PauliFeatureMapLayer(
                self.num_qubits, self.paulis, self.entanglement, self.alpha
            )
            self.layers[f"layer_{layer}"] = layer_ansatz
            count: int = layer_ansatz.get_parameter_count()
            layer_kernel: cudaq.Kernel = layer_ansatz.build_kernel()
            kernel.apply_call(layer_kernel, qvec, *x[ptr : ptr + count])
            ptr += count
        return kernel

    def get_parameter_count(self) -> int:
        r"""Return the total number of parameters in the Pauli feature map.

        Returns:
            int: Total parameter count across all layers.
        """
        layer_total: int = PauliFeatureMapLayer(
            self.num_qubits, self.paulis, self.entanglement, self.alpha
        ).get_parameter_count()
        return self.num_layers * layer_total
